	...
	public static void main(String[] args) {
		...
		while (opCode.compareTo("END") != 0) {
			...
			case "INC": {
				...
				// new syntax
				result = result.add(new BigInteger("1")).add(
						BigInteger.valueOf(operandOne));
				break;
			}
			case "DEC": {
				int operandOne = Integer.parseInt(codeArgs[1]);
				// new syntax
				result = result.subtract(new BigInteger("1")).add(
						BigInteger.valueOf(operandOne));
				break;
			}
			case "ADD": {
				...
				// new syntax
				result = result.add(BigInteger.valueOf(operandOne).add(
						BigInteger.valueOf(operandTwo)));
				break;
			}
			case "MLA": {
				int operandOne = Integer.parseInt(codeArgs[1]);
				...
				multiplied = multiplied
						.multiply(BigInteger.valueOf(operandTwo));
				result = multiplied;
				break;
			}
			default:
