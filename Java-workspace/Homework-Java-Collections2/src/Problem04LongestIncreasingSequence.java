...
public class Problem04LongestIncreasingSequence {
	...
	public static void main(String[] args) {
		...
		String[] lineSplited = scanner.nextLine().split(" ");
		int[] numbers = new int[lineSplited.length];		
		
		for (int i = 0; i < lineSplited.length; i++) {		//parsing string to integer
		...
		for (int i = 1; i < numbers.length; i++) {
			...
			if (counterSequence > counter) {		//with this we find the longest sequence ;)
		...
		int index = (indexNumber - (counter - 1));	//with this we find the start index of the longest sequence ;)
		//check the counter:
