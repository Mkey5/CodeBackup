...
<!---->
<!--<body>-->
<!--    <form action="RichPeople.php" method="post">-->
...
<html>
	<head>
		<meta charset="UTF-8"/>
		<title>Баси Големите Ръки ,чУек!</title>
		<style>
				table,tr,td,th{
					border: 1px solid black;
				}
	...
	<body>
		<form action="Problem02.php" method="post">
			<label for="input-field">Enter cars !</label>
			<input type="text" name="cars" id="input-field"/>
			<input type="submit" name="submit" id="submit" value="Show result"/>
		</form>
		<?php
			$colors = ['white','blue','green','yellow','black','red','purble','gray'];
	
			if (isset($_POST['submit']) && $_POST['cars'] != '') {
				
				echo '</br>';
				$cars = explode(",", $_POST['cars']);
				echo '<table><thead><tr><th>Cars</th><th>Color</th><th>Quantity</th></tr></thead>';
				echo '<tbody>';
